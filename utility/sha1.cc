...
unsigned int SHA1::shaToInteger( unsigned char* c, int l, unsigned int spacesize ) {
    assert( c );
    assert( l > 0 );
    ...
    while( l > 0 ) {
        int degree = l*2;
        sprintf(part, "%x", *c );
        result += hex_map[part[0]] * pow(16, degree);
        result += hex_map[part[1]] * pow(16, degree-1);
        l--;
        c++;
    }
    return (unsigned int) fmod(result,spacesize+1);
}
...
unsigned int SHA1::getIntSHA1(string str) {
    SHA1* sha1 = new SHA1();
